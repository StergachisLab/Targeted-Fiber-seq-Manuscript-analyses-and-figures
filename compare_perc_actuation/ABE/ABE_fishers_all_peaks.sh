...
pval_sample1_sample2=$(python -c "from scipy.stats import fisher_exact; fisher=fisher_exact([[$sample2_FIRE_count, $sample2_other_count], [$sample1_FIRE_count, $sample1_other_count]]); print(fisher[1])")


printf "$chr\t$peak\t$sample1_FIRE_count\t$sample1_other_count\t$sample2_FIRE_count\t$sample2_other_count\t$pval_sample1_sample2\n" >> peak_stats.txt

printf 'chr\tstart\tend\tsample1_FIRE\tsample1_other\tsample2_FIRE\tsample2_other\tpval_sample1_sample2\n' > ${output_prefix}peak_stats.txt
...
awk '$8 < 0.05 {print $1,$2,$3}' ${output_prefix}peak_stats.txt > ${output_prefix}peak_stats_sig.txt

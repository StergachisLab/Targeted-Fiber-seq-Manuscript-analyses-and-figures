#Checks for statistically significant difference between normal and RE alleles across each peak
...
chr=chr19
peak_path="/mmfs1/gscratch/stergachislab/bohaczuk/scripts/fiberseq-fire-0.0.4/FIRE/results/DM1all.norm.4/FDR-peaks/FDR-FIRE-peaks.bed.gz" # path to peaks bed output of fiberseq-fire
...
conda activate ft-pipeline # conda environment with samtools and fibertools
...
conda activate fiber-views # conda environment with scipy.stats



printf '$chr\tstart\tend\tnorm_FIRE\tnorm_other\tlarge_FIRE\tlarge_other\tpval_large\tsmall_FIRE\tsmall_other\tpval_small\n' >> peak_stats.txt
...
#Subtract from "other" any IDs that are also represented in FIREs
...
awk '$8<0.05' normal_peaks_widepeak_stats.txt | cut -f1-3 > largeRE_sig_peaks.bed

awk '$11<0.05' normal_peaks_widepeak_stats.txt | cut -f1-3 > shortRE_sig_peaks.bed


rm $norm_FIREs $large_FIREs $small_FIREs $norm_FIRE_IDs $norm_other_IDs $large_FIRE_IDs $large_other_IDs $small_FIRE_IDs $small_other_IDs peak_stats.txt

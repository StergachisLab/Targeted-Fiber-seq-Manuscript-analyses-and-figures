...
# Make a list of reads to exclude, m6Afrac is <0.02 or >0.3


# Add reads with m6Afrac <low_bound or >up_bound to list
exclude_reads=(data.loc[(data['m6Afrac']< float(low_bound)) | (data['m6Afrac']> float(up_bound)),'read_name'].tolist())

# write list to file
with open('exclude_list.txt','w') as f:
    for read in exclude_reads:
            read=read.split('/')
            f.write(read[1]+'\n')
'''